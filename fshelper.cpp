...
FSOpResult buffer2File(const unsigned char* buffer, const std::wstring filePath, const size_t bufferSize,
	const bool purgeFileIfExists) {
	FSHandler fsh;
	if(fsh.PathExists(filePath) ) {
		if (purgeFileIfExists) {
	...
	unsigned long bytesWritten = 0, bytes2Write = 0;
	size_t writePos = 0, szDiff = 0;
	NEW_ARR_NULLIFY(writeBuf, unsigned char, FS_WRITEBUFSZ);
	if (!writeBuf) {
		return FSOpResult::Fail;
	}
	do {
		szDiff = bufferSize - writePos;
		if (szDiff >= FS_WRITEBUFSZ) {
			bytes2Write = FS_WRITEBUFSZ;
		} else {
			bytes2Write = szDiff;
		}
		if (!::WriteFile(hFile, &buffer[writePos], bytes2Write, &bytesWritten, 0)) {
			return FSOpResult::Fail;
		}
		writePos += bytes2Write;
	} while(bufferSize > writePos);
...
FSOpResult file2Bufffer(unsigned char* &buffer, const std::wstring filePath) {
	...
	if (FSOpResult::Success != fsh.GetFSize(fsize, filePath)) {
		return FSOpResult::Fail;
	}
	if (fsize) {
		NEW_ARR_NULLIFY(readBuf, unsigned char, FS_READBUFSZ);
		if (readBuf) {
			NEW_ARR_NULLIFY_NO_REDEFINE(buffer, unsigned char, fsize);
			if (buffer) {
				/*::HANDLE hFile = ::CreateFile(filePath.c_str(), READ_CONTROL | FILE_READ_ATTRIBUTES,
					FILE_SHARE_READ, 0, OPEN_EXISTING, FILE_FLAG_SEQUENTIAL_SCAN, 0);*/
				::HANDLE hFile = ::CreateFile(filePath.c_str(), GENERIC_READ,
					FILE_SHARE_READ, 0, OPEN_EXISTING, FILE_FLAG_SEQUENTIAL_SCAN, 0);
				if (INVALID_HANDLE_VALUE != hFile) {
					unsigned long bytesRead = 0, readBufPos = 0;
					while ((::ReadFile(hFile, readBuf, FS_READBUFSZ, &bytesRead, 0)) && (readBufPos < fsize)) {
						if (!bytesRead) {
							break;
						}
						memcpy(&buffer[readBufPos], readBuf, bytesRead * sizeof(unsigned char));
...
DriveRecord::DriveRecord(DriveRecord &&other) noexcept {
	if (this != &other) {
		driveIndex = std::exchange(other.driveIndex, 0);
...
bool DriveRecord::operator==(const DriveRecord &other) {
	if (this != &other) {
		return (driveIndex == other.driveIndex &&
				physOffset == other.physOffset &&
...
std::wstring FSHandler::calcHash(const std::wstring filePath,
	...
	if (CryptAcquireContext(&hProv, NULL, NULL, PROV_RSA_AES, CRYPT_VERIFYCONTEXT)) {
		...
		if (::CryptCreateHash(hProv, (ALG_ID)hashType, 0, 0, &hHash)) {
			...
			if (FSOpResult::Success != file2Bufffer(fileContent, filePath)) {
				return L"";
			}
			if (fileContent) {
				unsigned long long fsize = 0;
