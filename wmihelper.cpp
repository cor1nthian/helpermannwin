	...
	if (WMIEnumSource::VBS == enumSource) {
			...
			if (avail) {
				...
				if (SysOpResult::Success == sys.GetSysTempFolderPath(tmpfld, root)) {
					...
					std::wstring out, errout, replacedquery =
						replaceAll_copy(gc_wmiCScriptNamespaceQuery, L"?replace?", namespaceName), tscriptpath =
		...
		if (SysOpResult::Success == sys.IsPowershellAvailable(avail)) {
			if (avail) {
				ProcessHandler proc;
				std::wstring out, errout, replacedquery =
					replaceAll_copy(gc_wmiPSNamespaceQuery, L"?replace?", namespaceName);
	...
	if (WMIEnumSource::VBS == enumSource) {
		...
		if (SysOpResult::Success == sys.IsCScriptAvailable(avail)) {
			if (avail) {
				...
				if (SysOpResult::Success == sys.GetSysTempFolderPath(tmpfld, root)) {
					FSHandler fsh;
					std::wstring out, errout, replacedquery = replaceAll_copy(gc_wmiCScriptClassesQuery,
		...
		if (SysOpResult::Success == sys.IsPowershellAvailable(avail)) {
			if (avail) {
				std::wstring replacedquery = replaceAll_copy(gc_wmiPSClassesQuery, L"?replace?", namespaceName);
				ProcessHandler proc;
